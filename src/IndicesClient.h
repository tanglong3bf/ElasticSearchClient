...
public:
    ...
    bool isAcknowledged() { return acknowledged_; }

    bool isShardsAcknowledged() { return shards_acknowledged_; }

    const std::string &getIndex() { return index_; }
...
public:
    ...
    const trantor::Date &getCreationDate() const { return creation_date_; }

    int32_t getNumberOfShards() const { return number_of_shards_; }

    int32_t getNumberOfReplicas() const { return number_of_replicas_; }

    const std::string &getUuid() const { return uuid_; }

    const std::string &getVersionCreated() const { return version_created_; }

    const std::string &getProvidedName() const { return provided_name_; }
...
public:
    ...
    const SettingsPtr &getSettings() const { return settings_; }
...
public:
    ...
    PutMappingResponsePtr putMapping(
      const std::string &indexName, const PutMappingParam &param
    ) const;
    void putMapping(
      const std::string                                     &indexName,
