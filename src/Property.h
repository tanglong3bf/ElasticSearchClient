...
public:
    ...
    Property(
      const std::string &property_name, PropertyType type, bool index = true
    )
      : property_name_(property_name),
        type_(type),
        index_(index) {
...
public:
    ...
    const PropertyType &getType() const { return type_; }

    bool getIndex() const { return index_; }

    const std::string &getAnalyzer() const { return analyzer_; }
